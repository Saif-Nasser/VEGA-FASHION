...
async function loginWithGoogle() {
    try {
        ...
        if (!window.auth || !window.googleProvider) {
            throw new Error("Google login not configured properly");
        }
        ...
        const isAdmin = await isUserAdminByUid(user.uid); return { success: true, user: user, isAdmin: isAdmin };
...
async function signUp(email, password, name) {
    try {
        ...
        if (!window.auth) {
            throw new Error("Firebase is not loaded. Check your Firebase configuration.");
        }
        ...
        if (error.code === 'auth/email-already-in-use') {
            ...
            errorMessage = "Network error. Please check your internet connection.";
        } else if (error.message && error.message.includes('Firebase is not loaded')) {
...
async function logIn(email, password) {
    try {
        ...
        if (error && error.code === 'auth/user-not-found') {
            ...
            errorMessage = "Network error. Please check your internet connection.";
        } else if (error && error.message && error.message.includes('Firebase is not loaded')) {
            errorMessage = "Authentication service is not available. Please refresh the page.";
            ...
            try {
                if (typeof error.message === 'string' && error.message.trim().startsWith('{')) {
                    ...
                    // Typical structure: { error: { message: 'INVALID_LOGIN_CREDENTIALS', ... } }
                    if (parsed && parsed.error && parsed.error.message) {
                        const apiMsg = parsed.error.message;
                        // Map common API-style codes to friendlier user messages
                        if (apiMsg === 'INVALID_LOGIN_CREDENTIALS' || /INVALID_?LOGIN|INVALID_CREDENTIALS|invalid login/i.test(apiMsg)) {
                            errorMessage = "Incorrect email or password. Please check your credentials and try again.";
                            console.debug('Parsed auth error token:', apiMsg);
                        } else {
                    ...
                    // Sometimes the error is already an object with .error
                ...
                // Couldn't parse JSON - fall back to message
